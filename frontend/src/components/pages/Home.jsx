  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      ...
      <section className="text-center py-20">
        ...
        />
        <h1 className="text-4xl font-bold font-poppin">Hi, I&apos;m Saurabh 👀</h1>
        <p className="text-lg mt-4 font-jost">MERN Developer | Blockchain Enthusiast | DevOps Learner</p>
      </section>

      {/* Skills Section */}
      <section className="text-center py-10">
        <h2 className="text-2xl font-semibold font-poppin">Skills & Tech Stack</h2>
      ...
      <section className="py-10">
        <h2 className="text-2xl font-semibold text-center font-poppin">Projects</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {projects.map((project, index) => (
            <div key={index} className="bg-gray-800 p-6 rounded-lg h-56 flex flex-col justify-between">
              <div>
                <h3 className="text-lg font-semibold font-poppin">{project.name}</h3>
                <p className="text-md text-gray-400 mb-2 font-jost">{project.description}</p>
                <p className="text-xs text-[#a4a7ac]"><span className="font-md font-semibold font-poppin text-[#a4a7ac]">Tech Stack:</span> {project.techStack}</p>
                <p className="text-xs text-[#a4a7ac]"><span className="font-md font-semibold font-poppin text-[#a4a7ac]">Type:</span> {project.type}</p>
              </div>
              <div className="flex flex-row justify-between mt-2 pl-10 pr-10">
                <a href={project.repo} className="text-blue-400 hover:underline"><Github size={"35"}/></a>
                <a href={project.site} className="text-green-400 hover:underline"><Globe size={"35"}/></a>
              </div>
            </div>
          ))}
        </div>
